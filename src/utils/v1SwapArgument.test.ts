...
describe('v1SwapArguments', () => {
  ...
  it('exact token to eth', () => {
    const trade = Trade.exactIn(new Route([USDC_WBNB], USDC, ETHER), new TokenAmount(USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('eth to exact token', () => {
    const trade = Trade.exactOut(new Route([USDC_WBNB], ETHER), new TokenAmount(USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('token to exact eth', () => {
    const trade = Trade.exactOut(new Route([USDC_WBNB], USDC, ETHER), CurrencyAmount.ether('100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
